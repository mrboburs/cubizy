...
<template>
    <div>
        ...
        <divloading :fullpage="false" :loading="loading" class="container-fluid">
            <div class="row justify-content-between">
                ...
                <div
                    ...
                    <template v-if="Object.keys(selected).length > 0">
                        <button v-for="action in bulk_actions" :key="action.key" type="button"
                            class="btn btn-sm  btn-outline-success m-1" :disabled="updating"
            ...
            <div class="table-responsive">
                <div class="theme_list d-flex flex-wrap p-1 gap-1">
                    <div v-for="record in records[currentPage]" :key="record.ID"
                        class="d-flex justify-content-between flex-column border border-4 rounded p-1"
                        :class="{ 'border-primary' : selected[record.ID] }" @click.prevent="select(record)">
                        <h6 v-if="title_column" class="text-center text-capitalize" style="max-width: 250px;">
                            {{record[title_column]}}
                        </h6>
                        <slot name="recordcard" v-bind:record="record">
                            <ul class="list-group">
                                <li v-for="(column, index) in visiblecolumns"
                                    :key="column.key + '_' + index + '_' + record.ID " class="d-flex flex-column">
                                    <label class="text-start text-capitalize">{{getColumnName(column)}}</label>
                                    <div>
                                        <slot :name="column.key" v-bind:row="record" v-bind:col="column">
                                            ...
                                            <template v-else-if="column.type == 'money'">
                                                <span class="text-nowrap">$ {{record[column.key]}}.00</span>
                                            </template>
                        ...
                        <div class="d-flex justify-content-between align-items-center p-1"
                            v-if="can_edit || can_delete || item_actions.length ">
                            <button v-for="action in item_actions" :key="action.key" type="button"
                                class="btn btn-sm btn-outline-primary border-0 me-1 flex-1" :disabled="updating"
                                @click.prevent="onItemAction(action.key, record)">
                                <i v-if="action.icon" :class="action.icon"></i>
                                <span v-if="action.text">{{ action.text}}</span>
                            </button>
                            <button v-if="can_edit" type="button" class="btn btn-sm btn-outline-success border-0 me-1"
                                :disabled="updating" @click.prevent="onEdit(record)">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button v-if="can_delete" type="button" class="btn btn-sm btn-outline-danger border-0 me-1"
                                :disabled="updating" @click.prevent="delete_item(record)">
        ...
        <v-offcanvas v-if="can_edit && editor_type == 'offcanvas'" v-model="editing" :placement="editor_size"
            :title="editing_item.ID? 'Edit Record': 'Add Record'" header-close-variant="light" title-class="font-18">
            <div v-if="message" class="alert d-flex align-items-center alert-dismissible fade show"
                :class="{'alert-success': !error, 'alert-danger': error }" role="alert">
                <strong v-if="!error">Success : </strong>
