...
var codesAPI = func(w http.ResponseWriter, r *http.Request, response *apiresponse.Response) error {
	var err error
	...
	if _, isFixCondition := response.Request["fix_condition"]; isFixCondition {
		...
		if _, ok := fixConditions["sub_locality"]; ok {
			sub_locality := util.GetString(fixConditions["sub_locality"])
			query.Where(" sub_locality = ? ", sub_locality)
		}
	}
	...
	if Search, okSearch := response.Request["search"]; okSearch {
		...
		if SearchString != "" {
			...
			query.Where(" locations.code Like ? ", SearchStringLike)
